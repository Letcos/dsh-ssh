// @dsh-ssh/dsh-ssh — unit tests (node --test, no network).
...
const KH_SAMPLE = [
  ...
  '[203.0.113.10]:2222 ssh-ed25519 ' + KEY_B,
  '# comment line',
  '',
...
test('parseKnownHosts parses hosts, ports, markers; skips comments/blank', () => {
  ...
  assert.equal(entries[1].hostPat, '[203.0.113.10]:2222');
...
test('knownHostsPatterns covers default and non-default ports', () => {
  ...
  assert.deepEqual(knownHostsPatterns('h', 2222), ['h', '[h]:2222']);
});

test('checkHostKey match/mismatch/unknown', () => {
  const entries = parseKnownHosts(KH_SAMPLE);
  assert.equal(checkHostKey('203.0.113.10', 22, BYTES_A, entries), 'match');
  assert.equal(checkHostKey('203.0.113.10', 2222, BYTES_B, entries), 'match');
...
// --- known_hosts: hashed entries + default path + missing file semantics ---
const HASH_SALT = Buffer.from('0123456789abcdef0123'); // 20 bytes, OpenSSH random salt length
...
test('checkHostKey matches OpenSSH hashed entries (HMAC-SHA1 host hash)', () => {
  ...
  assert.equal(checkHostKey(HASH_HOST, 22, BYTES_X, entries), 'mismatch'); // host matches, key mismatch
  assert.equal(checkHostKey('other.example', 22, BYTES_A, entries), 'unknown'); // host not matched
});
...
test('_readKnownHosts falls back to the default path when knownHostsPath is unset', async (t) => {
  // Isolate from the real ~/.ssh/known_hosts: point os.homedir() at a temp dir whose
  // .ssh/known_hosts does not exist, so the default-path fallback reads nothing ([] on ENOENT).
  const tmp = mkdtempSync(path.join(os.tmpdir(), 'dsh-ssh-kh-'));
...
test('makeHostVerifier records SshError host-key-unknown when host unknown', () => {
  const entries = parseKnownHosts(KH_SAMPLE);
  const conn = new SshConn({ id: 'h' });
...
// ── host-key-unknown structured error + idempotent known_hosts append ──
...
test('sshKeyFingerprint matches OpenSSH SHA256 (no padding, SHA256: prefix)', () => {
  assert.equal(sshKeyFingerprint(Buffer.alloc(0)), 'SHA256:47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU'); // known sha256('') value (SHA256 fingerprint is standard base64 with + /)
...
test('sshKeyTypeFromBlob parses the leading algorithm string of an SSH pubkey blob', () => {
  ...
  assert.equal(sshKeyTypeFromBlob(BYTES_A), ''); // no length prefix -> parse failure returns ''
...
test('knownHostsLine picks the non-default-port hostpat ([host]:port) and plain host otherwise', () => {
  ...
  assert.equal(knownHostsLine('h', 2222, 'ssh-ed25519', 'KEY'), '[h]:2222 ssh-ed25519 KEY');
});
...
test('appendKnownHost writes <host> <keytype> <base64>, is idempotent, and newline-safe (temp file)', async () => {
  ...
  try {
    ...
    // 2) Idempotent: same host+keytype+key not appended again
    ...
    // 3) Newline handling: when file ends with \n, appending second key does not create blank line
    ...
    // 4) Different hosts append independently
...
// SshConn.exec with a fake channel stream: verifies the maxStdoutBytes guard.
  ...
  const stream = {
    ...
    on(ev, cb) {
      ...
      if (ev === 'data') setImmediate(() => {
        ...
        if (handlers.close) handlers.close(0, null); // Close after data to avoid hanging the event loop
      });
      return stream;
    },
...
// SftpWrapper.readBytes pipeline: simulates out-of-order responses (EOF before data);
// completion is driven by the stat size, so an early EOF does not truncate the read.
test('SftpWrapper.readBytes handles out-of-order EOF responses', async () => {
  ...
  const fakeRaw = {
    ...
    read: (handle, buf, off, len, position, cb) => {
      if (position >= data.length) {
        setImmediate(() => cb(null, 0)); // EOF response returns immediately
      } else {
        setTimeout(() => { // Delayed data response to simulate EOF arriving first
