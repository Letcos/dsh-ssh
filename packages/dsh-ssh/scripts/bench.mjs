#!/usr/bin/env node
// @dsh-ssh/dsh-ssh benchmark: real numbers against the test remote (real data source for README).
// Measures: connection setup time / exec round-trip latency / SFTP read-write throughput / remote glob / remote grep.
// Uses only remote /tmp (dsh-ssh-bench-<pid>) and cleans up afterwards; does not touch settings.yaml or user known_hosts.
...
// Host/private-key config comes from live-config: defaults to ubuntu@203.0.113.10 / id_ed25519, fully overridable via environment variables.
const cfg = {
...
let conn = null;
try {
  // 1. Connection setup time (cold connection)
  ...
  // 2. exec round-trip latency (20x 'true')
  const execMs = [];
  for (let i = 0; i < 20; i++) {
  ...
  // 3. Prepare remote test tree (10 dirs x 20 files, with some grep-able content)
  ...
  // 4. SFTP read throughput (1 MiB)
  await conn.exec(`rm -f bench-1m.bin`); // clean up possible leftover
  ...
  // 5. SFTP write throughput (1 MiB, atomic write)
  ...
  // 6. remote glob (find implementation, 200-file tree)
  {
    const t = performance.now();
  ...
  console.log(`\n远端: ${liveConfig.user}@${liveConfig.host}:${liveConfig.port}, 本机: ${os.hostname()} (${os.platform()}/${os.arch()})`);
} finally {
  if (conn) { try { await conn.exec(`rm -rf ${shellQuoteSingle(remoteRoot)}`); } catch { /* best effort */ } }
  await pool.dispose();
}
