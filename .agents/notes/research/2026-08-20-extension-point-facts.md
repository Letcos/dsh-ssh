...
> 本文件由主编代理在等待调研子代理期间独立验证, 与 2026-08-20-dsh-official-reference.md(调研子代理产出)互为交叉验证。所有条目注明出处。日期: 2026-08-16。
...
- **M1 验收链已实测(2026-08-16, 主编代理, 用 /tmp 探针包 + 隔离 profile dsh-ssh-dev)**: ① `dsh plugin --profile dsh-ssh-dev add <本地路径>` → pnpm 以 link: 依赖安装并**自动把包名追加进 dsh.profile.bundles**; ② `dsh --profile dsh-ssh-dev --dump-config` → 组合整棵 profile 树并退出(exit 0), 输出可见 `# == <包名>` 段与 insert 行; ③ `dsh plugin --profile dsh-ssh-dev remove <包名>` → 干净卸载, bundles 恢复。注意: `dsh plugin` 子命令即转发 pnpm; add 本地路径无构建授权问题。
...
## 6.5 开发环境事实(2026-08-16 主编探测)

- 本机 sshd 未开启(仅 ssh-agent 在运行, 未开放本地 SSH 服务)。
- **主测试远端(用户提供)**: ubuntu@203.0.113.10:22[^rfc5737](公钥认证 OK; Linux x86_64; **exec + SFTP 均可用**)——M2/M3 的主验证目标。
- 次测试远端: user@203.0.113.10:<ssh-port>[^rfc5737](公钥认证 OK; 另一类 Unix 主机; exec 可用, SFTP 未开)——用于 exec-only 场景验证。
- **远端 SFTP 未开启**: sshd_config 中 `#Subsystem sftp <sftp-server>` 被注释(<sftp-server> 二进制存在)——exec 通道正常可用, M2 不受影响; M3 文件工具(SFTP 主路径)前需用户在远端取消注释并重启 sshd。
- 探测用的 known_hosts 隔离在 /tmp/dsh-ssh-test-known_hosts(不污染用户 ~/.ssh/known_hosts)。
- ssh2 npm 最新版 = 1.17.0(engines node>=10.16; 依赖仅 asn1 + bcrypt-pbkdf)——与可行性报告选型一致。

## 6.5.5 部署时间线

- 2026-08-16: 经用户确认, dsh-ssh 已安装进真实 web profile(bundles = [dsh-base, dsh-web-app, dsh-ssh]); web dump-config 确认 dsh-ssh 行(maxConnections: 4)进入组合。运行中的 GUI 需重启 + 刷新页面后生效。回滚: dsh plugin --profile web remove dsh-ssh。

## 6.6 客户端(client half)契约(M2b 预研, 2026-08-16 主编核对)
...
- **DSH_HOME 环境变量覆盖已实测(2026-08-16)**: `DSH_HOME=/tmp/<dir> dsh ...` 将 profiles/ 等全部用户数据重定向到指定目录(出处: lib/profile-boot 注释"may be set by the test or launcher after import"; 实测: 隔离目录内 initProfile + plugin add + dump-config 全链路成功)。**M3 起的 preset/settings/工作区实验一律用一次性 DSH_HOME, 不污染真实 ~/.dsh(GUI 正在使用)**。
...
[^rfc5737]: RFC 5737 示例地址，非真实主机；主机名与端口为占位符。
