...
## Decision
- 全量删除:packages/dsh-ssh/preset/(agent.cordis.yml, preset.yml);packages/preset-standard-ssh/ 整个包(上游快照/build/validate/install-preset);packages/旧 scope 包/ 整个包(已确认为 preset 副本);scripts/install-preset.mjs;test/preset-auto-install.test.js。
