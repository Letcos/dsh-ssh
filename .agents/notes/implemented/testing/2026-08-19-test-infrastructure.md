...
- **统一 live 配置 `test/live-config.mjs`**:一处定义 host/port/user/hostId/keyPath/remoteRoot,全部支持环境变量覆盖 `DSH_SSH_TEST_HOST/PORT/USER/HOST_ID/KEY_PATH/REMOTE_ROOT`;默认 = RFC 5737 TEST-NET-3 保留示例地址 203.0.113.10(公网永不路由, 仅供文档占位); 真实主机必须由 DSH_SSH_TEST_HOST 显式指定。所有 live/verify/bench 脚本(_live-jobs/live-background-verify/bench/live-smoke/tools-live-smoke/sandbox-live-verify/verify-*/functional-live-test)改从这里读,顺带修掉私钥默认不一致(id_rsa vs id_ed25519)。
