...
>
> 本文件是代理的**总入口**, 只放上手所需的最少信息; 架构细节与调研一律下沉到 `.agents/notes`(见 §5)。
...
> 原始验证流水见 `.agents/notes/archived/`。动代码前先复核。
...
- **禁止在注释里写入过程化信息**: 里程碑编号、内部方案编号、笔记条目号(如 A.42)、以及"修复/补充/曾经"式的历史叙述。这类内容归属 `.agents/notes` 与 `git log`, 不应出现在代码注释中。
...
- 笔记目录结构与条目组织规则见 `.agents/notes/README.md`, 操作规则见 `.agents/notes/AGENTS.md`。

## 5. 协作规范

1. 开工前必读: 根目录 `AGENTS.md` + `.agents/notes` 下与本任务相关的笔记。
2. 任务优先拆给**子代理并行推进**; 独立执行用 **deepseek-v4-flash**, 需承接完整上下文的用 fork 型子代理。
3. 子代理任务结束, 把**关键结论 / 已验证事实 / 踩坑**回写到 `.agents/notes`(注明出处: URL 或源码路径)。
